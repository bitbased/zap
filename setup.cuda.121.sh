...
# The following sections are commented out because the base image
# pytorch:2.1.0-py3.10-cuda11.8.0-devel-ubuntu22.04 already provides
# the CUDA toolkit, GLVND, Mesa, and EGL runtime.
...
apt install -y libxi-dev libgl1-mesa-dev libegl1-mesa-dev libgles2-mesa-dev build-essential python3 pkg-config
...
if [ "${RUN_HOST}" = "true" ]; then
  ...
  nginx -g 'daemon off;' &
  HEADLESS_GL_PLATFORM=egl exec ts-node src/zap-host.ts
fi
